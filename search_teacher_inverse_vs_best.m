function results = search_teacher_inverse_vs_best(mode_str)
if nargin < 1
    mode_str = 'quick';
end
...
out_dir = fullfile(pwd, ['teacher_inverse_vs_best_' timestamp]);
...
fprintf('Teacher-inverse search output:\n%s\n', out_dir);
...
fprintf('Saved teacher-inverse search to:\n%s\n', out_dir);
end
...
for roi_margin = roi_margin_list
    ...
    for lambda_reg = lambda_list
        for embed_scale = embed_list
            w_cfg = struct('lambda_reg', lambda_reg, 'normalize_rows', true, 'embed_scale', embed_scale);
            [W, ~] = teacher_inverse_beamformer_ula(cfg.M, design_points, cfg.d_over_lambda, w_cfg);
            for fine_step = fine_step_list
                count = count + 1;
                search_space(count).label = sprintf('inverse-roi%.1f-l%1.0e-e%.1f-h%.2f', roi_margin, lambda_reg, embed_scale, fine_step);
...
if numel(design_points) ~= P
    design_points = interp1(1:numel(design_points), design_points, linspace(1, numel(design_points), P), 'linear');
    for k = 1:numel(true_doa_deg)
        [~, idx] = min(abs(design_points - true_doa_deg(k)));
        design_points(idx) = true_doa_deg(k);
    end
    design_points = sort(design_points);
end
end
...
fprintf(fid, 'Teacher inverse W + codebook decoder search\n');
