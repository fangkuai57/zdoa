function results = search_teacher_hybrid_vs_best(mode_str)
if nargin < 1
    mode_str = 'quick';
end
...
out_dir = fullfile(pwd, ['teacher_hybrid_vs_best_' timestamp]);
...
fprintf('Teacher-hybrid search output:\n%s\n', out_dir);

results.narrow = run_one_case(get_narrow_case(), 'narrowband', cfg, 'center_dense');
results.wide = run_one_case(get_wide_case(), 'wideband', cfg, 'uniform');

write_summary(results, fullfile(out_dir, 'summary.txt'));
save(fullfile(out_dir, 'results.mat'), 'results', '-v7.3');
fprintf('Saved teacher-hybrid search to:\n%s\n', out_dir);
end

function case_result = run_one_case(scene, modality, cfg, steer_name)
fprintf('\n=== %s | %s ===\n', modality, scene.name);
old_ref = evaluate_old_reference(scene, modality, cfg, steer_name);

search_space = build_hybrid_search_space(scene, modality, cfg, steer_name);
records = struct([]);
best_idx = 1;
for i = 1:numel(search_space)
    rec = evaluate_candidate(scene, modality, cfg, search_space(i));
...
function old_ref = evaluate_old_reference(scene, modality, cfg, steer_name)
W = build_steer_beamformer(cfg.M, cfg.P, cfg.scan_deg, steer_name, cfg.d_over_lambda);
...
old_ref.label = ['old-' steer_name];
old_ref.avg_mse_deg2 = mean(mse_values);
old_ref.mse_curve = mse_values;
old_ref.snr_values = snr_values;
end

function search_space = build_hybrid_search_space(scene, modality, cfg, steer_name)
W_steer = build_steer_beamformer(cfg.M, cfg.P, cfg.scan_deg, steer_name, cfg.d_over_lambda);
roi_margin_list = [1.5, 2.5];
lambda_list = [1e-4, 1e-3, 1e-2];
beta_list = [0.05, 0.10, 0.20, 0.35];
fine_step_list = [0.02, 0.05];
...
for roi_margin = roi_margin_list
    ...
    design_points = build_design_points(scene.true_doa_deg, roi_min, roi_max, cfg.P);

    for lambda_reg = lambda_list
        [W_teacher, ~] = teacher_inverse_beamformer_ula(cfg.M, design_points, cfg.d_over_lambda, struct('lambda_reg', lambda_reg, 'normalize_rows', true, 'embed_scale', 1.0));
        for beta = beta_list
            W_hybrid = normalize_rows((1 - beta) * W_steer + beta * W_teacher);
            for fine_step = fine_step_list
                count = count + 1;
                label = sprintf('hybrid-%s-roi%.1f-l%1.0e-b%.2f-h%.2f', steer_name, roi_margin, lambda_reg, beta, fine_step);
                search_space(count).label = label;
                search_space(count).W = W_hybrid;
                search_space(count).decode_cfg = make_decode_cfg(cfg, roi_min, roi_max, fine_step, scene, design_points);
                if strcmp(modality, 'wideband')
                    search_space(count).freq_W_bands = repmat(W_hybrid, 1, 1, numel(scene.freq_ratios));
                else
...
function rec = evaluate_candidate(scene, modality, cfg, candidate)
snr_values = cfg.search_snr_values;
mse_values = zeros(size(snr_values));
for s = 1:numel(snr_values)
    trial_mse = zeros(1, cfg.trials);
...
function design_points = build_design_points(true_doa_deg, roi_min, roi_max, P)
base = linspace(roi_min, roi_max, max(P - numel(true_doa_deg), 2));
points = sort(unique([base, true_doa_deg]));
if numel(points) > P
    keep = false(size(points));
    for k = 1:numel(true_doa_deg)
        [~, idx] = min(abs(points - true_doa_deg(k)));
        keep(idx) = true;
    end
    remain = find(~keep);
    need = P - sum(keep);
    idx_pick = round(linspace(1, numel(remain), need));
    keep(remain(idx_pick)) = true;
    points = points(keep);
elseif numel(points) < P
    extra = linspace(roi_min, roi_max, P);
    points = sort(unique([points, extra]));
    points = points(1:P);
end
design_points = sort(points(:)).';
end

function cfg_out = make_decode_cfg(cfg, roi_min, roi_max, fine_step, scene, design_points)
cfg_out = struct();
cfg_out.L = cfg.L;
cfg_out.d_over_lambda = cfg.d_over_lambda;
cfg_out.fit_floor = 1e-8;
cfg_out.max_alt_iters = 12;
cfg_out.fine_grid_deg = sort(unique([roi_min:fine_step:roi_max, design_points]));
cfg_out.fine_step_deg = fine_step;
cfg_out.pool_size = 18;
cfg_out.pool_guard_deg = max(0.10, 2 * fine_step);
cfg_out.refine_radii_deg = [max(0.16, 4 * fine_step), max(0.08, 2 * fine_step), max(0.04, fine_step)];
cfg_out.refine_samples = 9;
cfg_out.refine_guard_deg = max(0.12, 2 * fine_step);
...
fprintf(fid, 'Teacher hybrid W + codebook decoder search\n');
