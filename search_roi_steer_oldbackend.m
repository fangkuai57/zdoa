function results = search_roi_steer_oldbackend(mode_str)
if nargin < 1
    mode_str = 'quick';
end
...
out_dir = fullfile(pwd, ['roi_steer_vs_best_' timestamp]);
...
fprintf('ROI-steer search output:\n%s\n', out_dir);
...
fprintf('Saved ROI-steer search to:\n%s\n', out_dir);
end
...
shape_list = {'uniform', 'center_dense', 'edge_dense'};
    records = {};
    count = 0;

for m = roi_margin_list
    roi_min = max(cfg.scan_deg(1), min(scene.true_doa_deg) - m);
    roi_max = min(cfg.scan_deg(end), max(scene.true_doa_deg) + m);
    for s = 1:numel(shape_list)
        count = count + 1;
        candidate = build_roi_candidate(scene, cfg, modality, m, shape_list{s}, roi_min, roi_max);
        ...
        fprintf('%s | avg MSE = %.4f deg^2 | %.3f dB\n', records{count}.label, records{count}.avg_mse_deg2, 10 * log10(records{count}.avg_mse_deg2));
    end
end
...
W = build_steer_beamformer(cfg.M, cfg.P, cfg.scan_deg, shape_name, cfg.d_over_lambda);
...
function candidate = build_roi_candidate(scene, cfg, modality, roi_margin, shape_name, roi_min, roi_max)
...
candidate.label = sprintf('roi-%s-m%.1f', shape_name, roi_margin);
candidate.W = W;
candidate.backend = 'old';
...
beam_deg = design_beam_grid_local(P, scan_deg(1), scan_deg(end), design_name);
W = zeros(P, M);
for p = 1:P
...
fprintf(fid, 'ROI steer + old backend search\n');
