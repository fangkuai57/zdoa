function results = search_mixed_roi_steer_vs_best(mode_str)
if nargin < 1
    mode_str = 'quick';
end
...
out_dir = fullfile(pwd, ['mixed_roi_steer_vs_best_' timestamp]);
...
fprintf('Mixed ROI steer search output:\n%s\n', out_dir);
...
fprintf('Saved mixed ROI steer search to:\n%s\n', out_dir);
end

function case_result = run_one_case(scene, modality, cfg, base_name)
fprintf('\n=== %s | %s ===\n', modality, scene.name);
old_ref = evaluate_candidate(scene, modality, cfg, build_baseline_candidate(scene, cfg, base_name));

margin_list = [2.5, 3.5, 4.5];
n_local_list = [10, 12, 14, 16];
local_shape_list = {'uniform', 'center_dense'};
records = {};
count = 0;

for margin = margin_list
    roi_min = max(cfg.scan_deg(1), min(scene.true_doa_deg) - margin);
    roi_max = min(cfg.scan_deg(end), max(scene.true_doa_deg) + margin);
    for n_local = n_local_list
        for s = 1:numel(local_shape_list)
            count = count + 1;
            candidate = build_mixed_candidate(scene, modality, cfg, margin, roi_min, roi_max, n_local, local_shape_list{s});
            rec_eval = evaluate_candidate(scene, modality, cfg, candidate);
            candidate.avg_mse_deg2 = rec_eval.avg_mse_deg2;
            candidate.mse_curve = rec_eval.mse_curve;
            records{count} = candidate; %#ok<AGROW>
            fprintf('%s | avg MSE = %.4f deg^2 | %.3f dB\n', candidate.label, candidate.avg_mse_deg2, 10 * log10(candidate.avg_mse_deg2));
        end
    end
end
...
function candidate = build_baseline_candidate(scene, cfg, shape_name)
...
candidate.label = ['baseline-' shape_name];
candidate.W = W;
candidate.backend = 'old';
candidate.freq_W_bands = repmat(W, 1, 1, numel(scene.freq_ratios_if_any));
end

function candidate = build_mixed_candidate(scene, modality, cfg, margin, roi_min, roi_max, n_local, local_shape)
beam_deg = design_mixed_beam_grid(cfg.P, cfg.scan_deg, roi_min, roi_max, n_local, local_shape);
W = build_W_from_beams(beam_deg, cfg.M, cfg.d_over_lambda);
candidate.label = sprintf('mixed-%s-m%.1f-n%d', local_shape, margin, n_local);
candidate.W = W;
candidate.backend = 'old';
...
function beam_deg = design_mixed_beam_grid(P, scan_deg, roi_min, roi_max, n_local, local_shape)
n_local = min(max(n_local, 2), P - 2);
n_outer = P - n_local;
n_left = floor(n_outer / 2);
n_right = n_outer - n_left;

local_beams = design_beam_grid_local(n_local, roi_min, roi_max, local_shape);
left_beams = [];
right_beams = [];
if n_left > 0 && roi_min > scan_deg(1)
    left_beams = linspace(scan_deg(1), roi_min, n_left + 2);
    left_beams = left_beams(1:end-2);
end
if n_right > 0 && roi_max < scan_deg(end)
    right_beams = linspace(roi_max, scan_deg(end), n_right + 2);
    right_beams = right_beams(3:end);
end

beam_deg = sort([left_beams, local_beams, right_beams]);
if numel(beam_deg) < P
    extra = linspace(scan_deg(1), scan_deg(end), P);
    beam_deg = sort(unique([beam_deg, extra]));
    beam_deg = beam_deg(1:P);
elseif numel(beam_deg) > P
    beam_deg = beam_deg(1:P);
end
end
...
function W = build_W_from_beams(beam_deg, M, d_eff)
...
scene.freq_ratios_if_any = scene.freq_ratios;
end
...
fprintf(fid, 'Mixed ROI steer + old backend search\n');
